...
}elseif($_REQUEST['formulario']=="agenda_semanal")
{
	...
	echo "</table>";
		...
		{
			$texto1="<H2>(E+C / T+O) MODERADO</H2>".$valores_evaluacion['general'];
			$color1="#FFAA00";
		}
...
elseif($_REQUEST['formulario']=="plan_personal_carrera_2")
{
	...
	echo "<table cellpadding='2' cellspacing='2'>";
        ...
        echo utf8_decode("<tr><td bgcolor='#CECECE'>Materia</td><td bgcolor='#CECECE'>Parcial1</td><td bgcolor='#CECECE'>Parcial2</td><td bgcolor='#CECECE' >Parcial3</td><td bgcolor='#CECECE'>Finales</td></tr>");
	...
	foreach($materias_cursar_filtro as $clave=>$curso){
            ...
            foreach($curso as $campo){
                ...
                if($cantidad==0){
                   echo "<td bgcolor='#CECECE'>".utf8_decode($arr_materias[$campo])."</td>";
                }  else {
                    echo "<td bgcolor='#CECECE'>".$campo."</td>";
                }
                $cantidad=1;
            }
            echo "</tr>";
...
$resultado = $sysacad->obtenerMaterias($id_alumno);
		...
		foreach($resultado as $mate)
		{
			...
			echo "<li>";	
			echo utf8_decode($resultado2[0]['nombre']);
			echo "</li>";
		}
		echo "</ul>";
