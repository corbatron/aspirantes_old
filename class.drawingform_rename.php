...
echo '<label class="checkbox">';
			...
			$input='<input  class="css-checkbox" name="'.$nombre.'" '.$estado.' id="'.$id.'" value="1" type="checkbox">';	
			echo $input.$texto;	
			echo '</input>';
echo '</label>';
	...
	{
		$input='<textarea class="form-control" name="'.$nombre.'"  id="'.$id.'" value="'.$nombre.'" rows="'.$rows.'" cols="'.$cols.'" wrap="virtual">Ninguno</textarea>';
		echo $input;
	}	
	...
	function DrawFormQuestions($nombre,$id,$id_formulario)
	{
		...
		$select_descripcion_larga_form="select descripcion_larga, descripcion from formularios where id=".$id_formulario;
...
*/
		...
		echo '<div class="well well-small"><h1>';
		echo $resultado_Dlarga[0]['descripcion'];
		echo '<img src="img/new_ppc.png" style="height:50Px;" class="img-circle"></h1></div>';
	...
	{
		echo '<input class="btn btn-info" type="submit" value="'.$texto.'">';
	}

	function DrawPlanCarrera()
	{
		
		include('plan_carrera.php');
